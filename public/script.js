...
function renderLatexContent() {
  const latexContent = {
    whatIsTaylor: `A Taylor series is a way to represent a function as an infinite sum of polynomials, centered at a point \\( a \\). It’s a powerful tool for approximating complex functions (like \\( \\sin(x) \\), \\( \\cos(x) \\), or \\( e^x \\)) using polynomials, which are easier to compute, differentiate, and integrate. As explained in <a href="https://www.youtube.com/watch?v=3d6DsjIBzJ4" target="_blank">3Blue1Brown’s video</a>, Taylor series are crucial in fields like physics and engineering. For example, approximating \\( \\cos(\\theta) \\approx 1 - \\frac{\\theta^2}{2} \\) simplifies pendulum problems by making equations more manageable.`,
...
<ul>
  <li>\\( f(0) = \\sin(0) = 0 \\), \\( f'(0) = \\cos(0) = 1 \\), \\( f''(0) = -\\sin(0) = 0 \\), \\( f'''(0) = -\\cos(0) = -1 \\).</li>
...
This ensures \\( P_n(x) \\) “spoons” with \\( f(x) \\) at \\( x = a \\), as 3Blue1Brown describes.`,
  ...
  Object.entries(latexContent).forEach(([id, latex]) => {
    try {
      katex.render(latex, document.getElementById(id), {
        throwOnError: false,
        displayMode: id === 'generalFormula'
      });
    } catch (e) {
      console.error(`Error rendering LaTeX for ${id}:`, e);
    }
  });
}
...
function updateGraph() {
  ...
  // Configure axes
  const isTrig = func === 'sin(x)' || func === 'cos(x)';
  const tickvals = isTrig ? [-Math.PI, -Math.PI/2, 0, Math.PI/2, Math.PI] : [-4, -2, 0, 2, 4];
  const ticktext = isTrig ? ['-\\pi', '-\\frac{\\pi}{2}', '0', '\\frac{\\pi}{2}', '\\pi'] : tickvals.map(v => v.toString());
  const aspectRatio = isTrig ? { x: Math.PI, y: 1 } : { x: 1, y: 1 };
  ...
  const layout = {
    ...
    xaxis: {
      title: 'x',
      range: isTrig ? [-Math.PI, Math.PI] : [-4, 4],
      gridcolor: '#555',
    ...
    yaxis: {
      title: 'y',
      range: isTrig ? [-2, 2] : [-5, 5],
      gridcolor: '#555',
      zerolinecolor: '#e0e0e0',
      zerolinewidth: 2,
      tickfont: { color: '#e0e0e0' },
      scaleanchor: isTrig ? 'x' : null,
      scaleratio: isTrig ? Math.PI : 1
    },
    plot_bgcolor: '#252525',
...
function generateSinPolynomialLatex(n, a) {
  ...
  for (let k = 0; k <= n; k++) {
    ...
    const denom = k === 0 ? '' : k === 1 ? '' : `\\frac{${coef}}{${factorials[k]}}`;
